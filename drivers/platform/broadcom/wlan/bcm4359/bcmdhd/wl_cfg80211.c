 ...
 *
 * $Id: wl_cfg80211.c 718503 2017-08-31 02:32:33Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	if ((err = wldev_ioctl_get(dev, WLC_GET_BSSID, &bssid, sizeof(bssid)))) {
		...
		if (!wldev_ioctl_get(dev, WLC_GET_CHANNEL, &ci, sizeof(ci))) {
	...
	if (dev_role == NL80211_IFTYPE_AP) {
		...
		if (is_rsdb_supported) {
			if ((err = wl_cfg80211_add_del_bss(cfg, dev,
				...
				if ((err = wldev_ioctl_set(dev, WLC_SET_AP,
						&ap, sizeof(s32))) < 0) {
					WL_ERR(("setting AP mode failed %d \n", err));
					err = -ENOTSUPP;
					goto exit;
...
s32 wl_cfg80211_up(void *para)
{
	...
	interr = wldev_ioctl_set(bcmcfg_to_prmry_ndev(cfg), WLC_SET_VAR, iovbuf, sizeof(iovbuf));
	if (unlikely(interr))
