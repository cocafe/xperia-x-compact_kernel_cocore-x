 ...
 *
 * $Id: dhd_linux.c 718503 2017-08-31 02:32:33Z $
 */
...
void dhd_set_scb_probe(dhd_pub_t *dhd)
{
	...
	ret = dhd_iovar(dhd, 0, "scb_probe", NULL, 0, (char *)&scb_probe, sizeof(scb_probe), FALSE);
	if (ret < 0) {
	...
	ret = dhd_iovar(dhd, 0, "scb_probe", (char *)&scb_probe, sizeof(wl_scb_probe_t),
			NULL, 0, TRUE);
	if (ret < 0) {
...
#if defined(DISABLE_PARALLEL_SCAN)
	ret = dhd_iovar(dhd, 0, "scan_parallel", (char *)&scan_parallel,
			sizeof(scan_parallel), NULL, 0, TRUE);
	if (ret < 0)
